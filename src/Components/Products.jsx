...
const Products = ({ setIsOpenFilter }) => {
  ...
  useEffect(() => {
    for (let i = 0; i < product_list.length / 9; i++) {
      paginationBtn.push(<button key={i} className='pagination-button' onClick={() => getPagination(i + 1)}>{i + 1}</button>)
  ...
  useEffect(() => {
    const copyProduct = [...product_list]
      ...
      if (selectedFilter == "low-high") {
        ...
        if (selectedFilter == "high-low") {
          ...
          if (selectedFilter == "a-z") {
            ...
            setFilteredProducts(copyProduct.sort((a, b) => a.name.localeCompare(b.name)))
          } else

            if (selectedFilter == "z-a") {

              setFilteredProducts(copyProduct.sort((a, b) => b.name.localeCompare(a.name)))
            } else {
  ...
  return (
    <>
      <ProductTitle setIsOpenFilter={setIsOpenFilter} setSelectedFilter={setSelectedFilter} />
      ...
      <div className='home-menu' id='home-menu'>
        ...
        {isOpenModal && <ProductModal product={modalData} setIsOpenModal={setIsOpenModal} />}
      </div>
